...
const CustomDnsCard = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (finalInvalid.some(Boolean)) {
      toast.error("请先修正高亮的 DNS 服务器地址。");
      return;
    }

    if (emptyWhenEnabled) {
      toast.error("请至少添加一个 DNS 服务器，或关闭自定义 DNS。");
      return;
    }
    ...
    if (success) {
      markSaved();
      toast.success(
        "自定义 DNS 已应用，局域网客户端在缓存刷新时可能会短暂中断。"
      );
    }
  };
  ...
  if (isLoading) {
    return (
      <Card className="@container/card">
        <CardHeader>
          <CardTitle>自定义上游 DNS</CardTitle>
          <CardDescription>
            覆盖 dnsmasq 转发到的运营商 DNS。局域网客户端仍查询调制解调器，只改变上游解析器。
          </CardDescription>
          <CardAction>
      ...
      {!available && (
        <Alert>
          ...
          <AlertDescription>
            ...
            ，自定义 DNS 不可用。通常表示调制解调器未使用 PROXY DNS 配置，请在网络设置中切换 DNS 模式后再启用。
          </AlertDescription>
        </Alert>
      )}
        ...
        <CardHeader>
          ...
          {passthroughBypass && (
            <p className="mt-2 text-sm text-muted-foreground">
              IP 透传正在绕过 dnsmasq；此设置只影响仍通过调制解调器转发 DNS 的客户端。
            </p>
          )}
        </CardHeader>
                      ...
                      <div className="flex flex-col gap-5 pt-1">
                        <Field>
                          <div className="flex items-center justify-between gap-2">
                            <FieldLabel>上游 DNS 服务器</FieldLabel>
                        ...
                        <Field orientation="horizontal">
                          ...
                          <div className="flex flex-1 flex-col gap-1 leading-snug">
                            ...
                            >
                              dnsmasq 只会转发到上方 DNS 服务器；这些服务器失败时不会回退到运营商 DNS。
                            </p>
                          </div>
                        </Field>
                      </div>
