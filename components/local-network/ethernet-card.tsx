...
const EthernetStatusCard = () => {
  ...
  const handleSpeedChange = async (value: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.detail || "设置链路速率失败");
      }
    } catch {
  ...
  const formatSpeed = (speed: string) => {
    if (!speed || speed === "Unknown") return "不可用";
  ...
  const formatDuplex = (duplex: string) => {
    if (!duplex || duplex === "Unknown") return "不可用";
  ...
  const formatNegotiationMode = (speedLimit?: string) => {
    if (!speedLimit) return "不可用";
    return speedLimit === "auto" ? "自动" : "手动";
  };
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup className="font-semibold text-muted-foreground @sm/card:text-base text-sm">
                    <SelectLabel>速率限制</SelectLabel>
                    <SelectItem value="auto">自动（最大）</SelectItem>
